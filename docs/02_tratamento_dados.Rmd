...
Forma sugerida de citação: Para fontes de diferentes setores ou dados globais acessados e baixados, utilize a seguinte citação: Climate TRACE (2025). Climate TRACE Emissions Inventory v4.2.0. Disponível em: <https://climatetrace.org>. Acessado em: [inserir data de acesso].
...
```{r}
emissions_sources_files <- list.files("../data-raw/BRA/",
             full.names = TRUE,
             recursive = TRUE,
           pattern = "emissions_sources.csv")
```
...
dados_emissoes <- dados_emissoes |> 
  dplyr::mutate(
    ...
    directory = stringr::str_remove(directory,"../data-raw/BRA/DATA/|_emissions_sources.csv"),
   )
```
...
Criando uma nova coluna chamada `biomes` que é uma versão "ajustada" do `biome`
Faz várias reclassificações baseadas em: Nome original do bioma (biome) e condições de latitude (lat) e longitude (lon).
