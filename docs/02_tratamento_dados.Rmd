  ...
  dplyr::mutate(
    ...
    source_type = ifelse(
      ...
      ifelse(stringr::str_detect(source_name,"Municipality"),
             "Municipality",
             ifelse(stringr::str_detect(source_name,"Urban Area"),
                    "Urban Area",NA)),source_type),
    source_name_trat = stringr::str_remove(source_name," Municipality| Urban Area"),
...
```{r}
dados_emissoes_urban_area <-  dados_emissoes |> 
  dplyr::filter(stringr::str_detect(source_name,"Urban Area"))

for(i in 1:nrow(dados_emissoes_urban_area)){
    source <- stringr::str_remove(dados_emissoes_urban_area[i,"source_name"]," Urban Area")
    df_aux <- dados_emissoes |> 
      dplyr::filter(stringr::str_detect(source_name,source)) |> 
      dplyr::group_by(lat) |> dplyr::n_groups()
     if((df_aux)>2) print(source)
}
